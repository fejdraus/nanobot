...
class TelegramConfig(Base):
    ...
    allowed_chats: list[str] = Field(default_factory=list)  # Allowed group/chat IDs
    admins: list[str] = Field(default_factory=list)  # Admin user IDs with full server access
    proxy: str | None = None
    ...
    # Fork default: streaming OFF (user preference — prefers whole messages over mid-stream edits).
...
class TelegramChannel(BaseChannel):
        ...
        for media_path in msg.media or []:
            try:
                ...
                sender = {
                    ...
                    # animation (GIF) — форк-доповнення: Telegram шле гіфки
                    # окремим методом, інакше вони йдуть документом і не грають
    ...
    async def _process_message_update(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        # Check if user is admin (our fork feature)
        user_id_str = str(user.id)
        is_admin = user_id_str in self.config.admins or (
            user.username and user.username in self.config.admins
        )

        # Check if chat is in allowed_chats (our fork feature: everyone in allowed chats can use bot)
        chat_allowed = (
            str_chat_id in self.config.allowed_chats or str(chat_id) in self.config.allowed_chats
        )
