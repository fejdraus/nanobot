...
def test_add_cron_job_defaults_to_tool_timezone(tmp_path) -> None:
    ...
    with request_context(
        ...
        result = tool._add_job(None, "Morning standup", None, None, "0 8 * * *", None, None)
...
def test_add_at_job_uses_default_timezone_for_naive_datetime(tmp_path) -> None:
    ...
    with request_context(
        ...
        result = tool._add_job(
            None, "Morning reminder", None, None, None, None, naive.isoformat()
        )
...
def test_add_at_job_rejects_past_datetime(tmp_path) -> None:
    ...
    with request_context(
        ...
        result = tool._add_job(None, "Old reminder", None, None, None, None, "2020-01-01T09:00:00")
...
def test_add_at_job_rejects_datetime_equal_to_now(tmp_path, monkeypatch) -> None:
    ...
    with request_context(
        ...
        result = tool._add_job(None, "Deadline reminder", None, None, None, None, at)
...
def test_add_at_job_accepts_future_datetime(tmp_path) -> None:
    ...
    with request_context(
        ...
        result = tool._add_job(None, "Future reminder", None, None, None, None, future)
...
def test_add_job_rejects_multiple_schedule_fields(tmp_path) -> None:
    ...
    with request_context(
        ...
        result = tool._add_job(None, "Morning standup", None, 60, "0 8 * * *", None, None)

    assert result == "Error: exactly one of delay_seconds, every_seconds, cron_expr, or at is required"
    assert tool._cron.list_jobs() == []
...
def test_add_job_binds_current_session_key(tmp_path) -> None:
    ...
    with request_context(
        ...
        result = tool._add_job(None, "Morning standup", None, 60, None, None, None)
...
def test_add_job_requires_session_key(tmp_path) -> None:
    ...
    with request_context(RequestContext(channel="telegram", chat_id="chat-1")):
        result = tool._add_job(None, "Background refresh", None, 60, None, None, None)
...
def test_explicit_tz_still_applies_to_at_schedules(tmp_path) -> None:
    ...
    tool = _make_tool_with_tz(tmp_path, "Asia/Shanghai")
    naive = (datetime.now(timezone.utc) + timedelta(days=1)).replace(tzinfo=None, microsecond=0)
    with request_context(
        RequestContext(channel="telegram", chat_id="chat-1", session_key="telegram:chat-1")
    ):
        result = tool._add_job(
            None, "Alarm", None, None, None, "Europe/Kyiv", naive.isoformat()
        )

    assert result.startswith("Created job")
    job = tool._cron.list_jobs()[0]
    expected = int(naive.replace(tzinfo=ZoneInfo("Europe/Kyiv")).timestamp() * 1000)
    assert job.schedule.at_ms == expected
